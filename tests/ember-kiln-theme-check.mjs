...
const expectedConfigPaths = {
  base: "public/Assets/TileMaps/themes/ember-kiln/floor-base.png",
  lane: "public/Assets/TileMaps/themes/ember-kiln/floor-lane.png",
  spawn: "public/Assets/TileMaps/themes/ember-kiln/floor-spawn.png",
  wall: "public/Assets/TileMaps/themes/ember-kiln/wall.png",
  crate: "public/Assets/TileMaps/themes/ember-kiln/crate.png",
};

const theme = getArenaThemeById(" EMBER-KILN ");
const queryTheme = resolveArenaTheme("?arenaTheme=ember-kiln");
const config = JSON.parse(await readFile(path.join(projectRoot, "configs", "arena-theme-library.json"), "utf8"));
const configTheme = (config.themes ?? []).find((entry) => entry.id === "ember-kiln");
...
const pass = theme?.id === "ember-kiln"
  && theme.renderMode === "sprite"
  && theme.name === "Ember Kiln"
  && queryTheme.id === "ember-kiln"
  && ARENA_THEME_LIBRARY.filter((entry) => entry.id === "ember-kiln").length === 1
  && configTheme?.status === "active"
  && configTheme.renderMode === "sprite"
  && configTheme.assetFolder === "public/Assets/TileMaps/themes/ember-kiln"
