# Documentation Index

Use this index as the first stop for release, gameplay, visual, and operations documentation. Keep it focused on documents that are intended to guide future changes.

## Product and Gameplay

- [Architecture](architecture.md) - high-level client, worker, and game-loop structure.
- [Next Map Creation Guide](next-map-creation-guide.md) - practical checklist for creating new arena maps.
- [Ultimate Creation](UltimateCreation.md) - guidance for character ultimate abilities.
- [Game Resolution Evaluation](gameres-evaluation.md) - notes on game resolution and presentation tradeoffs.
- [Arena Tileset Research](arena-tileset-research.md) - visual research for arena tile direction.

## Visual and Content Production

- [Visual Design Rules](VisualDesignRules.md) - visual rules for game presentation work.
- [Animation Jobs](animation-jobs.md) - animation production and job notes.
- [Oficina Criativa Visual - Coleta](../DocsDev/oficina-criativa-visual-coleta.md) - inventario auditavel das artes promovidas, onde aparecem e como foram validadas.
- [Progress Log](progress.md) - historical implementation progress notes.

## Operations

- [Repository README](../README.md) - setup commands, release-oriented test gates, and code principles.
- [Oracle Cloud VM](oracle-cloud-vm.md) - VM deployment and operations notes.
- [Swarm Coordination](../DocsDev/swarm-coordination.md) - coordination notes for parallel agent work.
- [Audio Map](../DocsDev/mapa-de-audio.md) - audio inventory and implementation map.
- [Creative Backlog](../DocsDev/backlog-criativo.md) - creative backlog for future production work.

## Release Records

- [Release v0.4.1](../DocsDev/releases/release-v0.4.1.md)
- [PRD v0.4.0](../DocsDev/releases/PRD-v0.4.0.md)
- [Before/After v0.4.0](../DocsDev/releases/before-after-v0.4.0.md)
- [Release v0.4.0](../DocsDev/releases/release-v0.4.0.md)
- [Release v0.3.0](../DocsDev/releases/release-v0.3.0.md)
- [Billing Checkout Readiness](../DocsDev/releases/billing-checkout-readiness.md)
- [Release v0.2.0](../DocsDev/releases/release-v0.2.0.md)
- [Release v0.2.1](../DocsDev/releases/release-v0.2.1.md)
- [Release v0.2.2](../DocsDev/releases/release-v0.2.2.md)
- [Release v0.2.3](../DocsDev/releases/release-v0.2.3.md)
- [Release v0.2.4](../DocsDev/releases/release-v0.2.4.md)
- [Release v0.2.5](../DocsDev/releases/release-v0.2.5.md)

## CodeGraph

- [CodeGraph Inventory](../DocsDev/codegraph/inventory.md) - module inventory and risk notes.
- [CodeGraph Context](../DocsDev/codegraph/codegraph-context.md) - generated structural context.
- [CodeGraph Status](../DocsDev/codegraph/codegraph-status.txt) - last recorded index status.
- [CodeGraph Visual Map](../DocsDev/codegraph/codegraph-visual.html) - clickable module map.
